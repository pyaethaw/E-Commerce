...
public class ProductDAO {
	public List<Product> getAllProductByCategory(Connection con,int catId){
		...
		try {
			...
			while(set.next()) {
				product = new Product(set.getInt("id"),set.getInt("cat_id"),set.getInt("price"),
						set.getString("name"),set.getString("image"),set.getNString("description"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
		...
		try {
			...
			while(set.next()) {
				product = new Product(set.getInt("id"),set.getInt("cat_id"),set.getInt("price"),pCount,
						set.getString("name"),set.getString("image"),set.getNString("description"),name);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
