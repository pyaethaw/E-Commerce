...
public class AdminUserController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (action != null) {
			if (action.contentEquals("orderdetail")) {
				showUserOrderDetail(request, response,con,pw);
			}
		}
		if(con != null) {
			try {
	...
	private void showUserOrderDetail(HttpServletRequest request, HttpServletResponse response,Connection con,PrintWriter pw)
			throws ServletException, IOException {
		...
		Gson gson = new Gson();
		List<Order> ods=orderDAO.getOrderOfAUser(con,Integer.parseInt(user_id));
